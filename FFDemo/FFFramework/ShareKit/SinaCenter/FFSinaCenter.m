...
static FFSinaCenter *instance;

@interface FFSinaCenter ()
...
- (void)getSinaUserInfoRequest {
    ...
    [SinaAuthRequest requestParameters:@{@"access_token":self.authInfo.accessToken, @"uid":self.authInfo.openId} successBlock:^(FFBaseRequest *request) {
        if (WS.successBlock) {
            WS.successBlock();
        }
...
- (void)didReceiveWeiboResponse:(WBBaseResponse *)response {
    ...
    if (response.statusCode == WeiboSDKResponseStatusCodeSuccess) {
        if (isWBSendMsgResp) {
            ...
            FFAuthInfo *authInfo = [[FFAuthInfo alloc] init];
