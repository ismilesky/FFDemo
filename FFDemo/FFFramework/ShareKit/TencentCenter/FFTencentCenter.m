...
NSString * const TencentErrorDomain = @"TencentError";

static FFTencentCenter *instance;

@interface FFTencentCenter ()
...
@implementation FFTencentCenter
...
- (void)shareContentInfo:(ShareContentInfo *)contentInfo successBlock:(ShareSuccessBlock)successBlock cancelBlock:(ShareCancelBlock)cancelBlock failureBlock:(ShareFailureBlock)failureBlock {
    if (!contentInfo || contentInfo.contentType == ShareContentType_None) {
        if (failureBlock) {
            NSError *error = [NSError errorWithDomain:TencentErrorDomain code:-102 userInfo:@{ErrorDescKey: @"未设置分享内容"}];
            failureBlock(error);
        }
        return;
    }
...
- (void)handleSendResult:(QQApiSendResultCode)sendResult successBlock:(ShareSuccessBlock)successBlock cancelBlock:(ShareCancelBlock)cancelBlock failureBlock:(ShareFailureBlock)failureBlock {
    ...
    if (errorMsg) {
        [self executeFailureBlock:failureBlock errorDomain:TencentErrorDomain errorMsg:errorMsg errorCode:sendResult];
        return;
    }
}
...
- (void)tencentDidNotNetWork {
    if (self.failureBlock) {
        NSError *error = [NSError errorWithDomain:TencentErrorDomain code:-100 userInfo:@{ErrorDescKey:@"没有网络，请设置网络连接"}];
...
- (void)getUserInfoResponse:(APIResponse *)response {
    if (response.errorMsg) {
        if (self.failureBlock) {
            NSError *error = [NSError errorWithDomain:TencentErrorDomain code:-101 userInfo:@{ErrorDescKey:response.jsonResponse?:@""}];
            self.failureBlock(error);
        }
    } else {
        FFAuthInfo *auth = [[FFAuthInfo alloc] init];
        ...
        FFUser *user = [[FFUser alloc] initWithQQDict:response.jsonResponse];
